...
Update_Shell(){
	sh_new_ver="${sh_ver}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Taylor000/tool/master/vendor/scripts/veip007-dd-od.sh" && chmod +x dd-od.sh
	echo -e "脚本已更新为 Taylor000/tool vendor 版本[ ${sh_new_ver} ] !(注意：因为更新方式为直接覆盖当前运行的脚本，所以可能下面会提示一些报错，无视即可)" && exit 0
}
...
wget --no-check-certificate -qO InstallNET.sh 'https://raw.githubusercontent.com/Taylor000/tool/master/vendor/scripts/veip007-InstallNET.sh' && chmod a+x InstallNET.sh && wget -N --no-check-certificate https://raw.githubusercontent.com/Taylor000/tool/master/vendor/scripts/veip007/Network-Reinstall-System-Modify.sh && chmod a+x Network-Reinstall-System-Modify.sh
...
read N
case $N in
  0) wget -N --no-check-certificate "https://raw.githubusercontent.com/Taylor000/tool/master/vendor/scripts/veip007/dd-gd.sh" && chmod +x dd-gd.sh ;;
  ...
  18) wget -N --no-check-certificate "https://raw.githubusercontent.com/Taylor000/tool/master/vendor/scripts/veip007/dd-gd.sh" && chmod +x dd-gd.sh && ./dd-gd.sh ;;
  *) echo "Wrong input!" ;;
esac
